...
class Compare extends Action
{
    ...
    public function execute()
    {
        ...
        ->addFieldToFilter('entity_id', ['like' => $pageId]);

        if(!isset($pageId) || $revisions->getSize() == 0 || $revisions->getSize() < 2){
            $this->messageManager->addErrorMessage(__('You can only compare when there are 2 or more revisions.'));
            $resultRedirect->setPath('mgz_revision/page/index');
            return $resultRedirect;
        }
        $this->registry->register('current_page_revisions', $revisions);
        $this->_setActiveMenu("Magezon_Revisions::revisions");
        $resultPage->getConfig()->getTitle()->prepend(__('Page Revision Compare'));
        return $resultPage;
    }
    ...
    {
        return $this->_authorization->isAllowed('Magezon_Revisions::revision_page_compare');
    }
}
